        ...
        $.post(url, {
            ...
            */
            ...
            for (let i=0;i<data.length;++i) {
                let maxScoreForThisClass = Math.max(data[i].classprobabilty);
                if(maxScoreForThisClass>bestScore) {
                    match = data[i];
                    bestScore = maxScoreForThisClass;
                }
            }*/
            if(Math.max(...data[0].classprobabilty)<50){
            	 $("#resultHolder").hide();
                $("#divClassTable").hide();                
                $("#error").hide();
                $("#Not").show();
            ...
            if (data) {
                ...
                let classDictionary = data[0]. class_dict;
                for(let personName in classDictionary) {
                    let index = classDictionary[personName];
                    let proabilityScore = data[0].classprobabilty[index];
